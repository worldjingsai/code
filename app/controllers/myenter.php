...
class Myenter extends SB_controller{
    ...
    public function result($team_id) {
        ...
        if($_POST){
            $config = array(
                    'allowed_types' => 'rar|doc|docx|pdf|zip',
                    'upload_path' => FCPATH.'uploads/results/',
                    'encrypt_name' => true,
                    'max_size' => '10240'
            );
    
            if (empty($_FILES['userfile']['tmp_name'])) {
                return $this->myclass->notice('alert("请选择文件!");window.location.href="'.site_url("myenter/result/${team_id}").'";');
            }
            $date = date('Ymd');
            $config['upload_path'] = $config['upload_path'] . $date . '/';
            if(!is_dir($config['upload_path'])){
            ...
            if ($this->upload->do_upload()) {
                ...
                $image_data_temp = $this->upload->data();
                $file = $date . '/' . $image_data_temp['file_name'];
                $this->db->where('team_id',$team_id)->update('team', array('result_file'=>$file));
                $data['msg'] = '文件上传成功!';
                header("location:/myenter/fee/${team_id}");
                exit();
            } else {
            
                return $this->myclass->notice('alert("文件不存在或者不符合要求请修改!");window.location.href="'.site_url("myenter/fee/${team_id}").'";');
            }
        }
        if ($data['team']['result_file']) {
            $data['team']['result_file'] = '/uploads/result_file/' . $data['team']['fee_image'];
        }
        $data['title'] = '作品信息';
